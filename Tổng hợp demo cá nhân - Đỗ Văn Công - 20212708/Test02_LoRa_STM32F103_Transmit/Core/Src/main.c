...
#define TX																	// Uncomment for Reception
...
void Send_register(void){
	char id[4] = "1.1";
	char id_parent[2] = "1";
	char data_register[9], ACK_register[20];
	sprintf(data_register,"%s R %s",id_parent,id);
	int register_ok =0, ret=0;
	while (!register_ok)
	{
		lora_begin_packet(&lora);
		...
		char buf[10];
				if(ret){
					...
					buf[i] = '\0';
					sprintf(ACK_register,"%s\r\n",buf);
					if(strcmp(ACK_register, "Register OK!") == 0){
					HAL_UART_Transmit(&huart1,(uint8_t *)ACK_register,strlen(ACK_register),1000);
					}
				}
	}
}
